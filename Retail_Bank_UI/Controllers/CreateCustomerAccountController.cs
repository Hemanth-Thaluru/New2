...
namespace Retail_Bank_UI.Controllers
{
    ...
    public class CreateCustomerAccountController : Controller
    {
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    ...
                    if(res1.IsSuccessStatusCode)
                    {
                        ...
                        var result =  client.APIClient().GetAsync("https://localhost:44368/api/Account/createAccount/" + customerCreationStatus.CustomerId).Result;
        ...
        public async Task<IActionResult> GetAccount(int AccountId)
        {
            ...
            var result = await client.APIClient().GetAsync("https://localhost:44368/api/Account/GetAccount/" + AccountId);
            if (result.IsSuccessStatusCode)
            {
                var s = result.Content.ReadAsStringAsync().Result;
